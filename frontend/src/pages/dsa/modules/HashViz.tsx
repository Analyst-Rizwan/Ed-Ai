...
export default function HashViz(){
  ...
  const insert=async()=>{
    ...
    try {
      ...
      if(mode==="chaining"){
        setHighlighted([h]);await anim.sleep(400);
        ...
        while(t[idx].length>0&&probes<SIZE){
          setHighlighted([idx]);addLog(`Slot [${idx}] occupied → probe +1`,"warn");await anim.sleep(350);idx=(idx+1)%SIZE;probes++;
        }
        if(probes>=SIZE){addLog("Hash table full!","err");anim.setRunning(false);return;}
        setHighlighted([idx]);await anim.sleep(300);t[idx]=[k];setTable(t);setHighlighted([]);
  ...
  const search=async()=>{
    ...
    try {
      ...
      if(mode==="chaining"){
        ...
        let idx=h,probes=0;
        while(probes<SIZE){
          setHighlighted([idx]);await anim.sleep(350);
          if(table[idx].length===0){setLabel(`search(${k}) → ✗ not found`);addLog("not found","err");setTimeout(()=>setHighlighted([]),800);anim.setRunning(false);return;}
          if(table[idx][0]===k){setLabel(`<strong>search(${k})</strong> → ✓ found at [${idx}]`);addLog(`found at [${idx}]`,"ok");setTimeout(()=>setHighlighted([]),800);anim.setRunning(false);return;}
  ...
  const runDemo=async()=>{
    ...
    try {
      ...
      for(const v of demoVals){
        ...
        setHighlighted([h]);await anim.sleep(350);
        ...
        setHighlighted([]);await anim.sleep(300);
      }
      ...
      <Side>
        <div>
          ...
          <div style={{marginTop:6}}>
            <Select
              value={mode} onChange={(v)=>{if(!anim.running){setMode(v);reset();}}}
              options={[["linear","Linear Probe"],["chaining","Chaining"]]}
        ...
        {!anim.running ? (
          ...
          <Controls anim={anim} run={() => {}} reset={() => { anim.reset(); reset(); }} />
        )}
